...
local function checkConfigSettings()
	if type(DEBUG_ENABLED) ~= "boolean" then
		DEBUG_ENABLED = false
	end
	if type(MSG_PREFIX_COLOR) ~= "string" then
		return false, "Invalid value for MSG_PREFIX_COLOR, expected #hex value"
	end
	if type(MSG_COLOR) ~= "string" then
		return false, "Invalid value for MSG_COLOR, expected #hex value"
	end
	if type(MESSAGES_PREFIX) ~= "string" then
		return false, "Invalid value for MESSAGES_PREFIX, expected string"
	end
	if type(STORAGE_RES_NAME) ~= "string" then
		return false, "Invalid value for STORAGE_RES_NAME, expected string"
	end
	if type(SCAN_FOLDERS) ~= "table" then
		return false, "Invalid value for SCAN_FOLDERS, expected table"
	end
	local sc = 0
	for i=1, #SCAN_FOLDERS do
		local v = SCAN_FOLDERS[i]
		if type(v) == "string" then
			sc = sc + 1
		end
	end
	if sc == 0 then
		return false, "No valid folders to scan in setting 'SCAN_FOLDERS'"
	end
	if type(COMMAND_RESCAN) ~= "string" then
		return false, "Invalid value for COMMAND_RESCAN, expected string"
	end
	if type(COMMAND_EXTRACT_IMG) ~= "string" then
		return false, "Invalid value for COMMAND_RESCAN_USAGE, expected string"
	end
	if type(DEFAULT_FILE_AUTO_DOWNLOAD) ~= "boolean" then
		return false, "Invalid value for DEFAULT_FILE_AUTO_DOWNLOAD, expected boolean"
	end
	if type(SCAN_MODS) ~= "table" then
		return false, "Invalid value for SCAN_MODS, expected table"
	end
	for name, enabled in pairs(SCAN_MODS) do
		if type(name) ~= "string" then
			return false, "Invalid key in SCAN_MODS, expected string"
		end
		if type(enabled) ~= "boolean" then
			return false, "Invalid value in SCAN_MODS, expected boolean"
		end
	end
	if type(KICK_ON_DOWNLOAD_FAILS) ~= "boolean" then
		return false, "Invalid value for KICK_ON_DOWNLOAD_FAILS, expected boolean"
	end
	if type(DOWNLOAD_MAX_TRIES) ~= "number" then
		return false, "Invalid value for DOWNLOAD_MAX_TRIES, expected number"
	end
	if type(NANDO_CRYPT_ENABLED) ~= "boolean" then
		NANDO_CRYPT_ENABLED = false
	elseif NANDO_CRYPT_ENABLED then
		if type(NANDO_CRYPT_EXTENSION) ~= "string" then
			return false, "Invalid value for NANDO_CRYPT_EXTENSION, expected string"
		end
		if type(NANDO_CRYPT_FUNCTION) ~= "string" then
			return false, "Invalid value for NANDO_CRYPT_FUNCTION, expected string"
		end
	end
	if type(ASYNC_PRIORITY) ~= "string" then
...
local function init()
	if not checkConfigSettings() then return end
